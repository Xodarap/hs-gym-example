...
overfit :: (KnownNat i, KnownNat h1, KnownNat h2, KnownNat o)
        => Network i h1 h2 o
        -> [(R i, R o)]
        -> Int
        -> Network i h1 h2 o
overfit net d epochs = foldr (\_ n -> trainList 0.1 d n) net (replicate epochs ())
...
main = MWC.withSystemRandom $ \g -> do
  ...
  let trainingData = [
        (vector @4 [1.0, 0.0, 0.0, 0.0], vector @2 [1.0, 0.0]),
        (vector @4 [0.0, 1.0, 0.0, 0.0], vector @2 [1.0, 0.0]),
        (vector @4 [0.0, 0.0, 1.0, 0.0], vector @2 [1.0, 0.0]),
        (vector @4 [0.0, 0.0, 0.0, 1.0], vector @2 [1.0, 0.0])]
  putStrLn $ "Average loss: " ++ show (averageLoss trainingData net0)
  putStrLn "\nTraining network..."
  let trainedNet = trainList 0.1 trainingData net0
  putStrLn $ "Average loss: " ++ show (averageLoss trainingData trainedNet)
  putStrLn $ "Overfit network..."
  let overfitNet = overfit trainedNet trainingData 1000
  putStrLn $ "Average loss: " ++ show (averageLoss trainingData overfitNet)